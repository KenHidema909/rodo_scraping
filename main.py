...
# --- 1. データ整形関数（★ここを修正しました） ---
def clean_fixed_layout(df_raw):
    # 「墜落・転落」という文字列が含まれる行を探して、そこをヘッダー行とする
    header_row_index = -1
    for i in range(min(10, len(df_raw))): # 上から10行目までを探す
        row_values = df_raw.iloc[i].astype(str).tolist()
        if any("墜落・転落" in val for val in row_values):
            header_row_index = i
            break
            
    if header_row_index == -1:
        print("エラー: ヘッダー行（'墜落・転落'を含む行）が見つかりませんでした。")
    ...
    # ヘッダー行より下のデータを取得
    # （単位の「人」などが書かれた行を飛ばすため、ヘッダーの次の行から開始）
    df_clean = df_raw.iloc[header_row_index + 1:].copy()
    df_clean.columns = new_header
    
    # A列とB列を結合してインデックス（業種名）にする
    col_a = df_clean.iloc[:, 0].fillna('').astype(str).str.replace('\n', '').str.replace('\r', '').str.strip()
    col_b = df_clean.iloc[:, 1].fillna('').astype(str).str.replace('\n', '').str.replace('\r', '').str.strip()
    
    # B列が空の場合はA列のみ、そうでない場合はA+B
    df_clean.index = col_a + col_b
    
    # 余分な空白や改行を削除
    df_clean.index = df_clean.index.str.replace(r'\s+', '', regex=True)

    valid_rows_count = 0
    for idx in df_clean.index:
        if idx == "": continue # 空行はスキップして次へ
        if "（注）" in idx or "出典" in idx or "注" in idx: break # 注釈が来たら終了
        valid_rows_count += 1
    ...
    # 「建設業」という文字が含まれる行を抽出（完全一致ではなく、部分一致で柔軟に対応）
    df_clean = df_clean[df_clean.index.str.contains("建設業")]
...
if __name__ == "__main__":
    ...
    # VSCode等でのテスト用に、URLがなくてもスクレイピング処理は実行するように変更
    result = get_mhlw_latest_report()
    ...
    if result:
        ...
        if not df.empty:
            ...
            if not GAS_APP_URL:
                print("\nエラー: GitHub Secretsに GAS_APP_URL が設定されていないため、送信はスキップします。")
            else:
                send_to_gas(df, fetched_date, GAS_APP_URL)
        else:
            print("\nエラー: データフレームが空です。抽出条件を見直してください。")
